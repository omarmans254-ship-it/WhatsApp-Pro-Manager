  ...
  if (clearAllBtn) {
    clearAllBtn.addEventListener('click', async () => {
      if (!accounts || accounts.length === 0) {
        return showToast(currentLang === 'ar' ? 'لا توجد أرقام لمسحها!' : 'No numbers to clear!', 'info');
      }
      ...
      if (checkedBoxes.length > 0 && checkedBoxes.length < accounts.length) {
        isSelectedOnly = true;
        const idsToDelete = Array.from(checkedBoxes).map(cb => cb.dataset.id);
        accountsToDelete = accounts.filter(a => idsToDelete.includes(a.id));
      } else {
        accountsToDelete = [...accounts];
      }

      const confirmMsg = isSelectedOnly
        ? (i18n[currentLang].confirm_clear_selected || 'هل أنت متأكد من مسح الأرقام المحددة؟')
        : (i18n[currentLang].confirm_clear_all || 'هل أنت متأكد من مسح جميع الأرقام من القائمة؟');

      if (confirm(confirmMsg)) {
        showToast(currentLang === 'ar' ? 'جاري مسح الأرقام...' : 'Clearing numbers...', 'info');

        for (const acc of accountsToDelete) {
          try {
            await window.electronAPI.clearSession(acc.id);
          } catch (e) {}
        }

        const deleteIds = accountsToDelete.map(a => a.id);
        accounts = accounts.filter(a => !deleteIds.includes(a.id));
        ...
        const toastMsg = isSelectedOnly
          ? (i18n[currentLang].toast_cleared_selected || 'تم مسح الأرقام المحددة بنجاح!')
          : (i18n[currentLang].toast_cleared_all || 'تم مسح جميع الأرقام بنجاح!');

        showToast(toastMsg, 'success');
        addLog(i18n[currentLang].log_sys, isSelectedOnly ? 'تم مسح الأرقام المحددة من القائمة.' : i18n[currentLang].log_cleared_all, 'info');
      }
    });
  }
